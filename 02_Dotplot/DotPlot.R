...
# sort df on ref
alignments$refID = factor(alignments$refID, levels = sort(unique(alignments$refID))) # set order of refID
alignments = alignments[with(alignments,order(refID,refStart)),]
chromMax = tapply(alignments$refEnd, alignments$refID, max)
chromMin = tapply(alignments$refEnd, alignments$refID, min)
...
# plot
yTickMarks = (tapply(alignments$queryEnd2, alignments$queryID, max)+tapply(alignments$queryEnd2, alignments$queryID, min))/2
yTickMark = tapply(alignments$queryEnd2, alignments$queryID, max)

options(warn = -1) # turn off warnings
if (opt$similarity) {
  gp = ggplot(alignments) +
    geom_point(
      mapping = aes(x = refStart2, y = queryStart2, color = refID),
      size = 0.009
    ) +
    geom_point(
      mapping = aes(x = refEnd2, y = queryEnd2, color = refID),
      size = 0.009
    ) +
    geom_segment(
      aes(
    ...
    scale_x_continuous(breaks = (cumsum(as.numeric(chromMax)) - as.numeric(chromMax)/2),
  ...
  gp = ggplot(alignments) +
    geom_point(mapping = aes(x = refStart2, y = queryStart2, color = refID),
               size = 0.009) +
    geom_point(mapping = aes(x = refEnd2, y = queryEnd2, color = refID),
               size = 0.009) +
    geom_segment(aes(
      x = refStart2,
      xend = refEnd2,
